...
deployment checks were not rerun. GitHub's first run exceeded the default
five-second timeout in the full-workstation mocked solve test at 5.2 seconds;
the test now has an explicit 10-second integration-test budget.
...
The complete status log from source commit `e4ae8c6` is preserved below. It spans
multiple releases and includes superseded claims; it is evidence, not a work queue.
The earlier 2026-09-15 snapshot follows unchanged for provenance. Their overlap is
intentional so neither snapshot loses evidence.
...
## Earlier snapshot archived 2026-09-15

# Project status history

Archived on 2026-09-15. These are dated observations, not current instructions.
Read [current status](PROJECT_STATUS.md) first. Old next-step, server, dependency,
verification and deployment claims apply only to their recorded milestone.
The complete previous status file is preserved below.
...
## Guided user flow — implemented locally, 2026-09-09

Simplified the planner journey to prepare the night → review/adjust → publish/notify.
The page resumes its latest returned version, collapses setup for a current draft,
puts the schedule ahead of routine provenance, and folds history/metrics/technical
records away. Publication has a keyboard-focus handoff, job/unscheduled counts,
validation and delivery/export actions. Observed stale snapshots automatically
open fresh-draft controls and disable publication; server checks are unchanged.

Request intake and meeting-note extraction have separate visible views that stay
mounted, preserving unfinished manual fields and transcript text. Submitted proposal
links return to the request queue; approval gives a direct scheduling handoff.
Dirty request controls block open/new/refresh until saved, completed or discarded.
Unrecorded review notes cannot follow a different plan version, and notification
settings retain edits across version/night changes. Existing routes, API contracts,
approval and immutable scheduling rules are unchanged; no new dependency or migration.

Verification: full suite 596/66 files passed. Final affected suite 30 tests passed
after browser-driven stale-state and disclosure refinements; production build,
typecheck and lint passed. Added regression checks cover resume/disclosure/focus,
manual/notes switching and submission handoff, dirty request navigation, note
version binding, destination edit preservation and stale publication controls.
Independent review found two edit-preservation issues; fixes and re-review passed.
Production browser checks passed initial latest-version resume, stale draft → fresh
creation, schedule selection, keyboard focus into publication, and manual/notes
round-trip preservation. Final screenshots were inspected at 1440px and 640px;
no page overflow at 640/1280/1920px. Contractor console was clean; planner had only
the existing favicon 404. Temporary identities and exact fixture plans were removed;
all browser runs made zero provider calls and the 127.0.0.1:3101 preview is stopped.
Post-cleanup database parity passed. This UX pass did not rerun concurrency or
controlled-provider HTTP E2E checks because backend behavior was unchanged; prior
results above remain historical evidence, with fresh UI/browser coverage here.

Usability limits: no representative first-time-user study has been performed.
These guards cover workspace controls, not arbitrary cross-page/browser navigation.
#17's spoken-reader and actual 200% workflow gates, operator rules/data, geography
permission and live-provider evidence remain open. No deployment, commit or push.

## Saved conflict revision — implemented locally, 2026-09-09

Connected the first research recommendation to `/plans`: inspect requested-time
conflict evidence on approved saved facts, choose a validated alternative or pin,
review every placement/deferral change, and save a new immutable draft. Revisions
retain their parent ID and exact pins in the input digest and exports. Existing
saved views remain immutable; server recomputation and publication remain separate.
Stale/superseded bases and mismatched browser engine versions are blocked. Failed
saves retain choices, and open previews disable publication and workspace changes.
No new dependency, schema migration or solver was introduced.

Verification: all 592 tests across 66 files passed; required hosted database parity plus
61 integration checks and 4 real concurrency checks passed; all 4 production E2E
checks passed. The extended HTTP journey preserves a chosen alternative for an
approved contractor revision through server save, publication and scoped export.
Typecheck, lint and production build passed. The seven revision checks, lint and
build were rerun after a final counter-label wording correction. Read-only review
found the browser-engine mismatch; its fix and follow-up review passed.

Browser UAT against the production preview verified keyboard alternative activation,
change diff, publication blocking during preview, saving/reopening, publication and
JSON readback. M-001 moved from 00:00–01:00 to pinned 00:15–01:15; the parent remained
unchanged. Screenshots at 1440px and 640px were visually inspected; no page overflow
at 640/1280/1440/1920px. Headed screenshot capture timed out; headless Chrome capture
worked. The only browser console error was the existing favicon 404. This does not
complete #17's actual spoken-reader/200% workflow checks.

All temporary E2E/browser identities and their exact artifacts were cleaned; the
browser run made zero provider calls. Post-cleanup DB parity passed. Preview at
127.0.0.1:3101 is stopped; no server remains from this work. No deployment, commit
or push was made. Existing operator-data, geography-permission, accessibility and
live-provider evidence limitations remain; this is still a non-operational prototype.

## NebulaX product research — 2026-09-09

Added `NEBULAX_PRODUCT_RESEARCH.md`: official challenge index, primary-source
rail/maintenance/port comparisons, selected implementation review at `aa7395f`,
data requirements, proposed focus areas, demo/evaluation criteria and mentor
questions. The recommendation is to connect existing validated conflict repairs
to saved planning, then validate operator inputs and measure shadow-pilot value.
This is research, not an accepted architecture change or a reordering of #17–#21.

The event page's two possible-dataset labels have no linked planning document.
Operating rules, anonymized planning examples and local adoption assumptions
remain unvalidated. Existing geography permissions, release accessibility and
live-provider evidence limits remain open. Source/link and whitespace checks
cover this documentation change; no app tests, deployment, provider sends or
database mutations were performed. No local server was started.
