...
An `n2-standard-8` has eight vCPUs and 32 GiB, enough to start with eight solver
workers and a 24 GiB service memory ceiling. Measure actual CPU/memory on the
selected host before resizing. [Google machine types](https://docs.cloud.google.com/compute/docs/general-purpose-machines).
...
port 3000. Native search defaults to 20 seconds per scenario and up to eight
available CPU workers. A separate 35-second child wall limit includes Python
startup/model construction. The warm heuristic has a cooperative one-second
budget; one construction may overrun it. Three UI requests solve A/B/C sequentially.
