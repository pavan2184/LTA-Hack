...
The baseline migration defines 16 tables, all with RLS enabled and no application
policies. The owner-only seed and loader bypass RLS. Anonymous and authenticated
Data API clients have no planning-data access at this stage.
...
state. There are no plan-version, identity, request-submission, workforce,
notification or audit tables yet; issues #5 onward introduce those contracts.
