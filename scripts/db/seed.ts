/**
 * Seed Postgres with the planning facts, then prove it round-trips.
 *
 * The literals in `@railplan/core` are the source. This script writes them out
 * and immediately reads them back through the same loader the application will
 * use, comparing content digests. A seed that inserts rows and reports success
 * without reading them back has only proven the inserts did not error — which
 * is not the same as proving the database now says what the engine says.
 *
 *   npm run db:seed
 */
import {
...
async function main() {
  ...
  try {
    ...
    const actual = instanceDigest(loaded);
