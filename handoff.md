...
| --- | --- | --- |
| 2026-09-19 | Codex | Resolved PR #47 against current `main`, retaining the native CP-SAT judge path plus PR #48 candidate validation/ECLO-window regressions and PR #49 submission assets. Added all-or-nothing native result/ZIP generation, separate native/regression benchmark commands and a 10-second budget for the full-workstation mocked solve test. Verification: 1,021 tests passed with 79 database-dependent skips; 101 targeted TypeScript/UI/API tests and 12 Python native tests passed; typecheck, lint and production build passed; one-run regression benchmark passed 39/39 outcomes at public A 25.2 / B 30 / C 25.2. The build required a 4 GiB Node heap after the default 2 GiB limit exhausted; the first isolated-worktree attempt also failed because Turbopack rejected external dependency symlinks. The first GitHub run exceeded the old 5-second UI-test timeout at 5.2 seconds; no assertion failed. Database, controlled HTTP E2E, hosted deployment and browser upload/download checks were not rerun. |
