...
vi.mock("node:os", () => ({ availableParallelism: mocked.availableParallelism }));
...
beforeEach(() => {
  ...
  mocked.availableParallelism.mockReturnValue(32);
});
...
describe("native PS1 solver boundary", () => {
  it("defaults to 60 seconds and 16 workers with a 75-second native process limit", async () => {
    ...
    reply((payload) => {
      expect(payload.seconds).toBe(60);
      expect(payload.workers).toBe(16);
      return nativeResult(payload);
    });
    const result = await solveNative(instance, { scenario: "A" });
    expect(result.diagnostics.solver).toMatchObject({ workers: 16, searchSeconds: 60 });
    expect(timeout).toHaveBeenCalledWith(expect.any(Function), 75_000);
  });

  it("caps default workers at the CPU parallelism available to the service", async () => {
    mocked.availableParallelism.mockReturnValue(4);
    reply((payload) => {
      expect(payload.workers).toBe(4);
      return nativeResult(payload);
    });
    const result = await solveNative(instance, { scenario: "A" });
    expect(result.diagnostics.solver!.workers).toBe(4);
  });

  it("allows an internal 32-worker cloud benchmark configuration", async () => {
    reply((payload) => {
      expect(payload.workers).toBe(32);
      return nativeResult(payload);
    });
    const result = await solveNative(instance, { scenario: "A" }, { workers: 32, seconds: 60 });
    expect(result.diagnostics.solver).toMatchObject({ workers: 32, searchSeconds: 60 });
  });

  it.each([
    { workers: 0 }, { workers: 33 }, { workers: 1.5 },
    { seconds: 0 }, { seconds: 61 }, { seconds: Number.NaN },
    { warmStartMs: -1 }, { warmStartMs: 5_001 },
  ])("rejects an invalid internal native budget before starting work: %j", async (configuration) => {
    const heuristic = vi.spyOn(schedule, "solveInstance");
    await expect(solveNative(instance, { scenario: "A" }, configuration)).rejects.toThrow("Invalid internal native solver budget");
